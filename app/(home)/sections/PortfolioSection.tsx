...
export function PortfolioSection() {
  ...
  const projectStats = useMemo(() => {
    return projects.map((p, i) => {
      const seed = `${p.id}:${p.title}:${i}`
      const h1 = stableHashInt(`${seed}:likes`)
      const h2 = stableHashInt(`${seed}:views`)
      return {
        likes: 20 + (h1 % 100), // 20..119
        views: 100 + (h2 % 500), // 100..599
      }
    })
  }, [projects])
      ...
      >
        <div className="relative min-h-screen flex items-center justify-center p-8">
          ...
          <div
            ...
            style={{
              background: mounted
                ...
                `
                : `
                  radial-gradient(circle at 30% 50%, var(--theme-accent) 0%, transparent 40%),
                  radial-gradient(circle at 70% 30%, var(--theme-secondary) 0%, transparent 45%)
                `,
              transform: mounted ? `rotate(${time * 2}deg) scale(${1 + Math.sin(time) * 0.1})` : "rotate(0deg) scale(1)",
              transition: "background 3s ease-in-out",
            }}
          />
          ...
          <div
            ...
            style={{
              transform: mounted
                ? `translate(${Math.sin(time * 0.3) * 100}px, ${Math.cos(time * 0.3) * 50}px) rotate(${Math.sin(time * 0.2) * 5}deg)`
          ...
          {["Neural Artist", "Chaos Engineer", "Joy Architect"].map((badge, i) => (
            <Badge
              ...
              style={{
                ...
                color: "var(--theme-text)",
                transform: mounted
                  ? `translate(${200 + Math.cos(time * 0.5 + i * 2) * 150}px, ${200 + Math.sin(time * 0.5 + i * 2) * 150}px) rotate(${time * 10 + i * 45}deg)`
          ...
          <div
            ...
            style={{
              transform: mounted ? `translateX(-50%) translateY(${Math.sin(time * 0.8) * 20}px)` : "translateX(-50%) translateY(0px)",
            }}
          >
            <Link href="/contact">
              <AudioButton
        ...
        <section className="relative py-32 px-8 pb-64">
          <div
            ...
            style={{ transform: mounted ? `translateX(-50%) rotate(${Math.sin(scrollY * 0.01) * 2}deg)` : "translateX(-50%) rotate(0deg)" }}
          >
            <h2
              ...
              return (
                <Card
                  ...
                  style={{
                    ...
                    transform: mounted
                      ? `rotate(${project.rotation + Math.sin(time * 0.3 + i) * 3}deg) translateX(${Math.sin(time * 0.2 + i) * 10}px)`
                      : `rotate(${project.rotation}deg) translateX(0px)`,
                    zIndex: 10 + i,
                    backgroundColor: "var(--theme-card)",
                    borderColor: "var(--theme-border)",
                ...
                >
                  <div className="relative h-full overflow-hidden rounded-lg">
                    ...
                    <div
                      ...
                      style={{
                        background: mounted
                          ? `linear-gradient(${45 + Math.sin(time + i) * 30}deg, var(--theme-primary), var(--theme-secondary), var(--theme-accent))`
                          : "linear-gradient(45deg, var(--theme-primary), var(--theme-secondary), var(--theme-accent))",
                        transition: "background 3s ease-in-out",
                      }}
                    />
                    ...
                    <CardContent className="relative h-full p-8 flex flex-col justify-between text-white z-10">
                      <div>
                        ...
                        <h3
                          ...
                          style={{ transform: mounted ? `rotate(${Math.sin(time * 0.4 + i) * 2}deg)` : "rotate(0deg)" }}
                        >
                          {project.title}
                        </h3>
